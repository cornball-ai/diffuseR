...
ltx23_causal_conv3d <- torch::nn_module(
    ...
    self$conv <- torch::nn_conv3d(in_channels, out_channels,
                                  self$kernel_size,
...
ltx23_video_downsampler3d <- torch::nn_module(
    ...
    to_channels <- function(x) {
        x <- x$unflatten(5L, c(-1L, s[3]))$unflatten(4L, c(-1L, s[2]))$unflatten(3L,
            c(-1L, s[1]))
        # [B, C, F', st, H', sh, W', sw] -> [B, C, st, sh, sw, F', H', W']
        x$permute(c(1L, 2L, 4L, 6L, 8L, 3L, 5L, 7L))$flatten(start_dim = 2L,
            end_dim = 5L)
    }
...
ltx23_video_upsampler3d <- torch::nn_module(
    ...
    to_space <- function(x) {
        x <- x$reshape(c(dims[1], -1L, s[1], s[2], s[3], num_frames,
                         height, width))
...
ltx23_video_down_block3d <- torch::nn_module(
    ...
    self$resnets <- torch::nn_module_list(lapply(seq_len(num_layers), function(i) {
        ltx23_video_resnet_block3d(in_channels, in_channels,
                                   eps = resnet_eps,
                                   spatial_padding_mode = spatial_padding_mode)
    }))
...
ltx23_video_up_block3d <- torch::nn_module(
    ...
    if (in_channels != out_channels) {
        self$conv_in <- ltx23_video_resnet_block3d(in_channels, out_channels,
            eps = resnet_eps, spatial_padding_mode = spatial_padding_mode)
    }
