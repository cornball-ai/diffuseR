...
.ltx23_finfo_max <- function(dtype) {
    tryCatch(
             ...
             error = function(e) {
        switch(dtype$.type(), BFloat16 = 3.3895313892515355e38, Half = 65504,
               3.402823466e38)
    }
    )
}
...
ltx23_rotary_pos_embed_1d <- torch::nn_module(
    ...
    grid_1d <- torch::torch_arange(start = 0, end = pos - 1,
                                   dtype = torch::torch_float32(),
                                   device = device) / self$base_seq_len
...
ltx23_transformer_block_1d <- torch::nn_module(
    ...
    self$attn1 <- ltx23_attention(query_dim = dim,
...
ltx23_connector_transformer_1d <- torch::nn_module(
    ...
    self$rope <- ltx23_rotary_pos_embed_1d(self$inner_dim,
        base_seq_len = rope_base_seq_len, theta = rope_theta,
...
ltx23_map_connector_key <- function(key) {
    ...
    key <- gsub("video_embeddings_connector", "video_connector", key,
                fixed = TRUE)
