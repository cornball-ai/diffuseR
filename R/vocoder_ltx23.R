...
ltx23_downsample1d <- torch::nn_module(
  ...
  forward = function(x) {
    ...
    torch::nnf_conv1d(
      x, self$filter$expand(c(num_channels, -1L, -1L)),
...
ltx23_upsample1d <- torch::nn_module(
  ...
  initialize = function(ratio = 2L, kernel_size = NULL, window_type = "kaiser",
    ...
    if (window_type == "hann") {
      ...
      time_axis <- (torch::torch_arange(start = 0, end = self$kernel_size - 1,
        dtype = torch::torch_float32()) / ratio - width) * rolloff
...
ltx23_antialias_act1d <- torch::nn_module(
  ...
  initialize = function(channels, ratio = 2L, kernel_size = 12L) {
    ...
    self$downsample <- ltx23_downsample1d(ratio = ratio, kernel_size = kernel_size)
  },
  forward = function(x) {
...
ltx23_vocoder <- torch::nn_module(
  ...
  initialize = function(
    ...
    self$conv_in <- torch::nn_conv1d(in_channels, hidden_channels,
      kernel_size = 7L, stride = 1L, padding = 3L)
...
ltx23_causal_stft <- torch::nn_module(
  ...
  initialize = function(filter_length = 512L, hop_length = 80L, window_length = 512L) {
    ...
    self$forward_basis <- torch::nn_buffer(torch::torch_zeros(n_freqs * 2L, 1L, filter_length))
...
ltx23_mel_stft <- torch::nn_module(
  ...
  initialize = function(
    ...
    self$mel_basis <- torch::nn_buffer(torch::torch_zeros(num_mel_channels, num_freqs))
  },
  forward = function(waveform) {
...
ltx23_vocoder_with_bwe <- torch::nn_module(
  ...
  initialize = function(
    ...
    bwe_resnet_dilations = list(c(1L, 3L, 5L), c(1L, 3L, 5L), c(1L, 3L, 5L)),
    filter_length = 512L,
    hop_length = 80L,
    window_length = 512L,
    num_mel_channels = 64L,
