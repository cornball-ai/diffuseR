...
ltx23_rms_norm <- torch::nn_module(
  ...
  forward = function(x) {
    input_dtype <- x$dtype
    variance <- x$to(dtype = torch::torch_float32())$pow(2)$mean(dim = -1L, keepdim = TRUE)
...
#'
#' @export
ltx23_get_timestep_embedding <- function(
  timesteps,
  embedding_dim,
  ...
  exponent <- -log(max_period) * torch::torch_arange(
    start = 0, end = half_dim - 1,
    dtype = torch::torch_float32(), device = timesteps$device
  )
...
ltx23_combined_timestep_embed <- torch::nn_module(
  ...
  forward = function(timestep, hidden_dtype) {
    proj <- ltx23_get_timestep_embedding(
      timestep, 256L,
...
ltx23_ada_layer_norm_single <- torch::nn_module(
  ...
  initialize = function(embedding_dim, num_mod_params = 6L) {
    ...
    self$linear <- torch::nn_linear(embedding_dim, num_mod_params * embedding_dim, bias = TRUE)
  },
  forward = function(timestep, hidden_dtype) {
...
.ltx23_sdpa <- function(query, key, value, attention_mask = NULL, chunk_size = NULL) {
...
ltx23_transformer_block <- torch::nn_module(
  ...
  initialize = function(
    ...
    self$norm1 <- ltx23_rms_norm(dim, eps = eps, elementwise_affine = elementwise_affine)
  ...
  forward = function(
    ...
    attn_hidden_states <- self$attn1(
      ...
      perturbation_mask = if (self$perturbed_attn) perturbation_mask else NULL,
