...
gemma3_rotary_embedding <- torch::nn_module(
    ...
    freqs <- torch::torch_einsum("bs,d->bsd",
                                 list(position_ids_float, self$inv_freq$to(device = x$device)))
...
repeat_kv <- function(hidden_states, n_rep) {
    ...
    hidden_states <- hidden_states$unsqueeze(3L)$expand(c(batch, num_kv_heads,
            n_rep, seq_len, head_dim))
...
gemma3_decoder_layer <- torch::nn_module(
    ...
    self$input_layernorm <- gemma3_rms_norm(config$hidden_size,
...
gemma3_text_model <- torch::nn_module(
    ...
    list(last_hidden_state = hidden_states, hidden_states = all_hidden_states)
}
)
                ...
                if (all(as.integer(hf_tensor$shape) == as.integer(native_tensor$shape))) {
                    ...
                    if (verbose) {
                        message("Shape mismatch: ", native_name,
                                " (HF: ", paste(as.integer(hf_tensor$shape),
                                collapse = "x"),
...
tokenize_gemma3 <- function(tokenizer, text, max_length = 1024L,
    ...
    result <- encode_bpe(tokenizer = tokenizer, text = text,
...
encode_with_gemma3 <- function(prompts, model = NULL, tokenizer = NULL,
    ...
    if (is.character(model)) {
        model <- load_gemma3_text_encoder(model, device = device,
