#' Stable Diffusion 2.1 VAE decoder (anvl port of AutoencoderKL decode)
#'
#' anvl re-implementation of the SD 2.1 \code{AutoencoderKL} decode path:
#' the scalar \code{scaling_factor} latent rescale (host-side, see
#' \code{\link{yq_sd_vae_prepare}}), the 1x1 \code{post_quant_conv}, then
#' the standard AutoencoderKL decoder body (\code{conv_in}; a mid block of
#' ResNet, self attention, ResNet; four up blocks of three ResNet blocks
#' with nearest-2x upsampling on the first three; a GroupNorm-32 -> SiLU ->
#' \code{conv_out} output head). The architecture is identical to the
#' FLUX.2 VAE decoder apart from the 4-channel latent and the scalar
#' (rather than BatchNorm) latent normalization, so the block helpers
#' (\code{.yq_vae_resnet}, \code{.yq_vae_attention}, \code{.yq_vae_up_block})
#' and conv helpers are shared with \code{R/anvl_vae.R}. Decodes
#' \code{[B, 4, H, W]} latents to \code{[B, 3, 8H, 8W]} pixels in [-1, 1].
#'
#' @name anvl_sd_vae
NULL

# SD 2.1 VAE scaling factor (diffusers AutoencoderKL config).
.YQ_SD_VAE_SCALING <- 0.18215

#' Stable Diffusion 2.1 VAE decoder forward (anvl)
#'
#' Reuses the shared AutoencoderKL block helpers from \code{R/anvl_vae.R};
#' \code{anvl::jit()} the wrapper \code{function(z) yq_sd_vae_decode(z, w)}.
#' \code{z} must already be rescaled by the VAE scaling factor (see
#' \code{\link{yq_sd_vae_prepare}}).
#'
#' @param z AnvlArray \code{[B, 4, H, W]} latents, already scaling-factor
#'   rescaled.
#' @param w VAE weights pytree from \code{\link{yq_sd_vae_load_weights}}.
#'
#' @return AnvlArray \code{[B, 3, 8H, 8W]} pixels in [-1, 1].
#'
#' @export
yq_sd_vae_decode <- function(z, w) {
...
#' Prepare SD 2.1 latents for the VAE decoder (scaling factor)
#'
#' The diffusers SD pipeline divides latents by the VAE
#' \code{scaling_factor} (0.18215) before decoding
#' (\code{z = z / scaling_factor}); this applies that rescale, returning
#' the input to \code{\link{yq_sd_vae_decode}}. A scalar-broadcast divide,
#' so latents stay on device (no host round-trip).
#'
#' @param z AnvlArray \code{[B, 4, H, W]} latents from the sampling loop.
#' @param scaling_factor Numeric. VAE scaling factor (SD 2.1: 0.18215).
#'
#' @return AnvlArray \code{[B, 4, H, W]}, rescaled.
#'
#' @export
yq_sd_vae_prepare <- function(z, scaling_factor = .YQ_SD_VAE_SCALING) {
    z / anvl::nv_scalar(scaling_factor, "f32", device = anvl::device(z))
}

#' Load SD 2.1 VAE decoder weights into an anvl pytree
#'
#' Reads the decode half of the diffusers \code{AutoencoderKL} checkpoint
#' (F16 upcast to f32): \code{post_quant_conv}, the decoder body, and the
#' output head. Conv weights load raw \code{[out, in, kH, kW]} (what
#' \code{\link[anvl]{nv_conv2d}} expects); the attention linears transpose
#' to \code{[in, out]} for \code{\link[yunque]{yq_linear}}. Encoder and
#' \code{quant_conv} keys are skipped (txt2img needs only the decode path).
#' With \code{strict = TRUE} every \code{decoder.*} / \code{post_quant_conv.*}
#' key must be consumed exactly once (and no requested key may be absent),
...
#'
#' @return Weights pytree for \code{\link{yq_sd_vae_decode}}.
#'
#' @export
yq_sd_vae_load_weights <- function(path, device = "cpu", strict = TRUE) {
    ...
    if (strict) {
        ...
        if (length(extra)) {
            stop("SD21 VAE anvl load: ", length(extra),
                 " requested keys not in file, e.g. ",
                 paste(utils::head(extra, 5), collapse = ", "))
        }
        if (length(unused)) {
            stop("SD21 VAE anvl load: ", length(unused),
