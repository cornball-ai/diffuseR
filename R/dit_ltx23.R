...
ltx23_transformer <- torch::nn_module(
    ...
    self$audio_time_embed <- ltx23_ada_layer_norm_single(audio_inner_dim,
        audio_mod_params)
    ...
    if (!is.null(audio_encoder_attention_mask) &&
                                      audio_encoder_attention_mask$ndim == 2L) {
...
ltx23_map_dit_key <- function(key) {
    ...
    key <- gsub("av_ca_video_scale_shift_adaln_single",
                "av_cross_attn_video_scale_shift", key, fixed = TRUE)
