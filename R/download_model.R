...
download_model <- function(model_name = "sd21",
                           ...
                           unet_dtype_str = NULL, overwrite = FALSE,
                           show_progress = TRUE, download_models = FALSE) {
    # Normalize 'devices'
    if (is.character(devices) && length(devices) == 1) {
        if (model_name == "sdxl") {
            devices <- list(unet = devices, decoder = devices,
                            text_encoder = devices, text_encoder2 = devices,
                            encoder = devices)
        } else {
            devices <- list(
                            unet = devices,
