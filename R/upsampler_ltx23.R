...
ltx23_upsampler_res_block <- torch::nn_module(
  ...
  initialize = function(channels, mid_channels = NULL) {
    ...
    self$conv1 <- torch::nn_conv3d(channels, mid_channels, kernel_size = 3L, padding = 1L)
...
ltx23_load_upsampler <- function(path, device = "cuda", dtype = "bfloat16",
  ...
  torch::with_no_grad({
    for (key in keys) dests[[key]]$copy_(handle$get_tensor(key))
  })
  model$to(device = device)
  model$eval()
  if (verbose) message("Loaded latent upsampler (", length(keys), " tensors)")
  model
}
...
#'
#' @export
ltx23_adain_filter_latent <- function(latents, reference_latents, factor = 1.0) {
  if (factor == 0) {
    return(latents)
  }
  dims <- c(3L, 4L, 5L)
