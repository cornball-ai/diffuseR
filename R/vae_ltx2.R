...
ltx2_video_encoder3d <- torch::nn_module(
    ...
    initialize = function(
        ...
        # Input convolution
        self$conv_in <- ltx2_video_causal_conv3d(in_channels = self$in_channels,
                                                 out_channels = output_channel,
                                                 kernel_size = 3L, stride = 1L,
        ...
        for (i in seq_len(num_blocks)) {
            ...
            down_blocks[[i]] <- ltx2_video_down_block3d(in_channels = input_channel,
                                                        out_channels = output_channel,
        ...
        # Mid block
        self$mid_block <- ltx2_video_mid_block3d(in_channels = output_channel,
    ...
    forward = function(
        ...
        hidden_states <- hidden_states$reshape(c(batch_size, num_channels,
        ...
        # Python's [:, -1:] selects the last channel; R's [, -1,,,] excludes it
        last_channel <- hidden_states[, hidden_states$shape[2],,,, drop = FALSE]
        last_channel <- last_channel$`repeat`(c(1L,
                                                hidden_states$shape[2] - 2L,
                                                1L, 1L, 1L))
        hidden_states <- torch::torch_cat(list(hidden_states, last_channel),
                                          dim = 2)
...
ltx2_video_decoder3d <- torch::nn_module(
    ...
    initialize = function(
        ...
        # Input convolution
        self$conv_in <- ltx2_video_causal_conv3d(in_channels = in_channels,
                                                 out_channels = output_channel,
                                                 kernel_size = 3L, stride = 1L,
                                                 spatial_padding_mode = spatial_padding_mode)
        ...
        for (i in seq_len(num_blocks)) {
            ...
            up_blocks[[i]] <- ltx2_video_up_block3d(in_channels = input_channel,
                                                    out_channels = output_channel,
        ...
        self$norm_out <- per_channel_rms_norm()
        self$conv_act <- torch::nn_silu()
        self$conv_out <- ltx2_video_causal_conv3d(in_channels = output_channel,
    ...
    forward = function(
        ...
        hidden_states <- hidden_states$reshape(c(batch_size, - 1, p_t, p, p,
        ...
        hidden_states <- hidden_states$flatten(start_dim = 3,
                                               end_dim = 4) # F*p_t
...
ltx2_video_vae <- torch::nn_module(
    ...
    initialize = function(
        ...
        self$encoder <- ltx2_video_encoder3d(in_channels = in_channels,
        ...
        self$decoder <- ltx2_video_decoder3d(in_channels = latent_channels,
        ...
        # Compression ratios - count only stages that affect each dimension
        # downsample_type: "spatial" affects spatial, "temporal" affects temporal,
        # "spatiotemporal" affects both
        n_spatial <- sum(downsample_type %in% c("spatial", "spatiotemporal"))
        n_temporal <- sum(downsample_type %in% c("temporal", "spatiotemporal"))
        self$spatial_compression_ratio <- patch_size * 2L ^ n_spatial
        self$temporal_compression_ratio <- patch_size_t * 2L ^ n_temporal
...
#' Tiled encoding for large spatial dimensions
    tiled_encode = function(
        ...
        for (i in seq_along(rows)) {
            ...
            result_rows[[length(result_rows) + 1]] <- torch::torch_cat(result_row,
                                                                       dim = 5)
        }

        enc <- torch::torch_cat(result_rows,
                                dim = 4)[,,, 1:latent_height, 1:latent_width]
        enc
    },
    ...
    tiled_decode = function(
        ...
        for (i in seq(1, height, by = tile_latent_stride_height)) {
            ...
            for (j in seq(1, width, by = tile_latent_stride_width)) {
                ...
                tile <- self$decoder(z[, , , i:i_end, j:j_end], temb,
                                     causal = causal)
                row[[length(row) + 1]] <- tile
            }
            rows[[length(rows) + 1]] <- row
        ...
        dec <- torch::torch_cat(result_rows,
                                dim = 4)[,,, 1:sample_height, 1:sample_width]
        dec
    },
...
#' Internal decode with tiling support
    .decode = function(
        ...
        if (self$use_tiling &&
            (width > tile_latent_min_width ||
                height > tile_latent_min_height)) {
        ...
        if (self$use_slicing && z$shape[1] > 1) {
            if (!is.null(temb)) {
                decoded_slices <- lapply(seq_len(z$shape[1]), function(i) {
                        self$.decode(z[i:i, drop = FALSE], temb[i:i,
                                     drop = FALSE], causal = causal)
                    })
            } else {
                decoded_slices <- lapply(seq_len(z$shape[1]), function(i) {
...
load_ltx2_vae <- function(
    ...
    if (dir.exists(weights_path)) {
        ...
        weights_file <- file.path(vae_dir,
                                  "diffusion_pytorch_model.safetensors")
        if (!file.exists(weights_file)) {
            stop("Could not find diffusion_pytorch_model.safetensors in: ",
                 vae_dir)
        }
    ...
    if (!is.null(config)) {
        vae <- ltx2_video_vae(in_channels = config$in_channels %||% 3L,
                              out_channels = config$out_channels %||% 3L,
                              latent_channels = config$latent_channels %||% 128L,
                              block_out_channels = as.integer(config$block_out_channels %||% c(256,
                                                                                               512,
                                                                                               1024,
                                                                                               2048)),
                              decoder_block_out_channels = as.integer(config$decoder_block_out_channels %||% c(256,
                                                                                                               512,
                                                                                                               1024)),
                              layers_per_block = as.integer(config$layers_per_block %||% c(4,
                                                                                           6,
                                                                                           6,
                                                                                           2,
                                                                                           2)),
                              decoder_layers_per_block = as.integer(config$decoder_layers_per_block %||% c(5,
                                                                                                           5,
                                                                                                           5,
                                                                                                           5)),
                              spatio_temporal_scaling = as.logical(config$spatio_temporal_scaling %||% c(TRUE,
                                                                                                         TRUE,
                                                                                                         TRUE,
                                                                                                         TRUE)),
                              decoder_spatio_temporal_scaling = as.logical(config$decoder_spatio_temporal_scaling %||% c(TRUE,
                                                                                                                         TRUE,
                                                                                                                         TRUE)),
                              decoder_inject_noise = as.logical(config$decoder_inject_noise %||% c(FALSE,
                                                                                                   FALSE,
                                                                                                   FALSE,
                                                                                                   FALSE)),
                              downsample_type = config$downsample_type %||% c("spatial",
                                                                              "temporal",
                                                                              "spatiotemporal",
                                                                              "spatiotemporal"),
                              upsample_residual = as.logical(config$upsample_residual %||% c(TRUE,
                                                                                             TRUE,
                                                                                             TRUE)),
                              upsample_factor = as.integer(config$upsample_factor %||% c(2,
                                                                                         2,
                                                                                         2)),
...
load_ltx2_vae_weights <- function(
    ...
    # Get native parameter and buffer names
    native_params <- names(vae$parameters)
    native_buffers <- names(vae$buffers)

    # Build mapping from HF names to R names
    remap_vae_key <- function(key) {
        # Strip vae. prefix (Wan2GP format)
        key <- sub("^vae\\.", "", key)
        # Wan2GP uses res_blocks, R uses resnets
        key <- gsub("\\.res_blocks\\.", ".resnets.", key)
        key
    }

    # Wan2GP uses a flat block layout where upsamplers/downsamplers are separate
    # indexed blocks, while R nests them. Build a mapping table.
    # Decoder flat: 0=mid_resnets, 1=up0_upsamp, 2=up0_resnets, 3=up1_upsamp, ...
    # Encoder flat: 0=down0_resnets, 1=down0_downsamp, 2=down1_resnets, ...
    #               last_resnets=mid_block
    remap_flat_vae_key <- function(key) {
        key <- sub("^vae\\.", "", key)
        key <- gsub("\\.res_blocks\\.", ".resnets.", key)

        # Handle per_channel_statistics -> latents_mean/latents_std
        # Wan2GP flat weights use per_channel_statistics.* keys:
        #   mean-of-means  -> latents_mean (matches diffusers latents_mean)
        #   std-of-means   -> latents_std  (matches diffusers latents_std)
        if (grepl("^per_channel_statistics\\.", key)) {
            stat <- sub("^per_channel_statistics\\.", "", key)
            if (stat == "mean-of-means") return("latents_mean")
            if (stat == "std-of-means") return("latents_std")
            return(key) # skip other stats
        }

        # Decoder flat blocks
        m <- regmatches(key,
                        regexec("^decoder\\.up_blocks\\.(\\d+)\\.(.+)$",
                                key)) [[1]]
        if (length(m) == 3) {
            flat_idx <- as.integer(m[2])
            rest <- m[3]
            if (flat_idx == 0L) {
                # flat 0 = mid_block resnets
                return(paste0("decoder.mid_block.", rest))
            }
            # flat 1,3,5,... = upsamplers; flat 2,4,6,... = resnets
            r_block <- (flat_idx - 1L) %/% 2L
            if (flat_idx %% 2L == 1L) {
                # Upsampler: flat N.conv.conv -> up_blocks.X.upsamplers.0.conv.conv
                return(paste0("decoder.up_blocks.", r_block, ".upsamplers.0.",
                              rest))
            } else {
                # Resnets
                return(paste0("decoder.up_blocks.", r_block, ".", rest))
            }
        }

        # Encoder flat blocks
        m <- regmatches(key,
                        regexec("^encoder\\.down_blocks\\.(\\d+)\\.(.+)$",
                                key)) [[1]]
        if (length(m) == 3) {
            flat_idx <- as.integer(m[2])
            rest <- m[3]
            # Encoder: 0=down0_resnets, 1=down0_downsamp, 2=down1_resnets, ...
            # Last resnets block = mid_block
            # With 4 down blocks: flat 0,1,2,3,4,5,6,7 = resnets/downsamp pairs, flat 8 = mid
            # General: even = resnets, odd = downsampler
            r_block <- flat_idx %/% 2L
            if (flat_idx %% 2L == 0L) {
                # Resnets block - could be mid_block if it's the last one
                # We detect mid_block by checking if there's no downsampler after it
                # (i.e., this is the highest even index)
                return(paste0("encoder.down_blocks.", r_block, ".", rest))
            } else {
                # Downsampler: flat N.conv.conv -> down_blocks.X.downsamplers.0.conv.conv
                return(paste0("encoder.down_blocks.", r_block,
                              ".downsamplers.0.", rest))
            }
        }
    ...
    # Detect Wan2GP format by checking for vae. prefix on keys
    is_wan2gp <- any(grepl("^vae\\.", names(weights)))
    remap_fn <- if (is_wan2gp) remap_flat_vae_key else remap_vae_key

    # For Wan2GP encoder, we need to detect the mid_block (last resnets group)
    # Find the highest even flat encoder index to remap it to mid_block
    if (is_wan2gp) {
        enc_flat_indices <- as.integer(unique(sub("^vae\\.encoder\\.down_blocks\\.(\\d+)\\..*$",
                                                  "\\1",
                                                  grep("^vae\\.encoder\\.down_blocks\\.",
                                                       names(weights),
                                                       value = TRUE))))
        max_enc_flat <- max(enc_flat_indices)
        # The last even index is the mid_block
        enc_mid_flat_idx <- max_enc_flat
    }
    ...
    torch::with_no_grad({
            for (hf_name in names(weights)) {
                native_name <- remap_fn(hf_name)

                # For Wan2GP encoder: remap last flat resnets block to mid_block
                if (is_wan2gp &&
                    grepl(paste0("^encoder\\.down_blocks\\.",
                                 enc_mid_flat_idx %/% 2L, "\\."),
                          native_name)) {
                    native_name <- sub(paste0("^encoder\\.down_blocks\\.",
                                              enc_mid_flat_idx %/% 2L, "\\."),
                                       "encoder.mid_block.", native_name)
                }
                ...
                } else if (native_name %in% native_buffers) {
                    native_tensor <- vae$buffers[[native_name]]
                }

                if (!is.null(native_tensor)) {
                    ...
                    if (all(as.integer(hf_tensor$shape) == as.integer(native_tensor$shape))) {
                        ...
                        if (verbose) {
                            message("Shape mismatch: ", native_name, " (HF: ",
                                    paste(as.integer(hf_tensor$shape),
                                          collapse = "x"),
                                    " vs R: ",
                                    paste(as.integer(native_tensor$shape),
                                          collapse = "x"),
                                    ")")
                        }
                        skipped <- skipped + 1L
