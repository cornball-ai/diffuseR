...
ltx2_rotary_pos_embed_1d <- torch::nn_module(
    ...
    forward = function(batch_size, seq_len, device) {
        ...
        grid_1d <- torch::torch_arange(start = 0, end = seq_len - 1L, dtype = torch::torch_float32(), device = device)
        ...
        pow_indices <- torch::torch_pow(self$theta, torch::torch_linspace(start = 0.0, end = 1.0, steps = self$dim %/% num_rope_elems, dtype = freqs_dtype, device = device))
...
ltx2_transformer_block_1d <- torch::nn_module(
    ...
    initialize = function(dim, num_attention_heads, attention_head_dim,
        ...
        self$attn1 <- ltx2_attention(query_dim = dim, heads = num_attention_heads, kv_heads = num_attention_heads, dim_head = attention_head_dim, rope_type = rope_type)
...
ltx2_connector_transformer_1d <- torch::nn_module(
    ...
    initialize = function(num_attention_heads = 30L,
        attention_head_dim = 128L, num_layers = 2L,
        ...
        # 1D RoPE
        self$rope <- ltx2_rotary_pos_embed_1d(dim = self$inner_dim, base_seq_len = rope_base_seq_len, theta = rope_theta, double_precision = rope_double_precision, rope_type = rope_type, num_attention_heads = num_attention_heads)

        # Transformer blocks
        self$transformer_blocks <- torch::nn_module_list(lapply(seq_len(num_layers), function(i) {
                    ltx2_transformer_block_1d(dim = self$inner_dim, num_attention_heads = num_attention_heads, attention_head_dim = attention_head_dim, rope_type = rope_type)
                }))
    ...
    forward = function(hidden_states, attention_mask = NULL,
        ...
        if (!is.null(self$learnable_registers)) {
            if (sequence_length %% self$num_learnable_registers != 0L) {
                stop(sprintf("Sequence length %d must be divisible by num_learnable_registers %d", sequence_length, self$num_learnable_registers))
            }
...
ltx2_text_connectors <- torch::nn_module(
    "LTX2TextConnectors",
    initialize = function(caption_channels = 3840L, text_proj_in_factor = 49L,
        ...
        self$text_proj_in <- torch::nn_linear(in_features = caption_channels * text_proj_in_factor, out_features = caption_channels, bias = FALSE)

        # Video connector
        self$video_connector <- ltx2_connector_transformer_1d(num_attention_heads = video_connector_num_attention_heads, attention_head_dim = video_connector_attention_head_dim, num_layers = video_connector_num_layers, num_learnable_registers = video_connector_num_learnable_registers, rope_base_seq_len = connector_rope_base_seq_len, rope_theta = rope_theta, rope_double_precision = rope_double_precision, causal_temporal_positioning = causal_temporal_positioning, rope_type = rope_type)

        # Audio connector
        self$audio_connector <- ltx2_connector_transformer_1d(num_attention_heads = audio_connector_num_attention_heads, attention_head_dim = audio_connector_attention_head_dim, num_layers = audio_connector_num_layers, num_learnable_registers = audio_connector_num_learnable_registers, rope_base_seq_len = connector_rope_base_seq_len, rope_theta = rope_theta, rope_double_precision = rope_double_precision, causal_temporal_positioning = causal_temporal_positioning, rope_type = rope_type)
    },
...
#' @export
encode_text_ltx2 <- function(prompt, backend = "random", model_path = NULL,
    tokenizer_path = NULL, text_encoder = NULL,
    embeddings_file = NULL, api_url = NULL,
    max_sequence_length = 1024L,
    caption_channels = 3840L, device = "cpu",
    ...
    if (backend == "gemma3") {
        ...
        result <- encode_with_gemma3(prompts = unlist(prompt), model = text_encoder %||% model_path, tokenizer = tokenizer_path %||% model_path, max_sequence_length = max_sequence_length, device = device, dtype = dtype_str, verbose = FALSE)
        ...
        # Call HTTP API via system curl (no extra R packages needed)
        body <- jsonlite::toJSON(list(prompts = prompt, max_sequence_length = max_sequence_length), auto_unbox = TRUE)
        body_file <- tempfile(fileext = ".json")
        on.exit(unlink(body_file), add = TRUE)
        writeLines(body, body_file)
        result <- system2("curl", args = c("-s", "-f", "-X", "POST", "-H", "Content-Type: application/json", "-d", paste0("@", body_file), api_url), stdout = TRUE, stderr = TRUE)
        status <- attr(result, "status")
        if (!is.null(status) && status != 0L) {
            stop("Text encoding API failed (HTTP error)")
        }
        ...
        prompt_embeds <- torch::torch_randn(c(batch_size, max_sequence_length, packed_dim), device = device, dtype = dtype)
        prompt_attention_mask <- torch::torch_ones(c(batch_size, max_sequence_length), device = device, dtype = torch::torch_int64())

    } else {
        stop("Unknown backend: ", backend, ". Use 'gemma3', 'precomputed', 'api', or 'random'")
    }

    list(prompt_embeds = prompt_embeds, prompt_attention_mask = prompt_attention_mask)
}
...
#' @export
load_ltx2_connectors <- function(weights_path, config_path = NULL,
    text_proj_path = NULL,
    ...
    if (!is.null(config)) {
        connectors <- ltx2_text_connectors(caption_channels = config$caption_channels %||% 3840L, text_proj_in_factor = config$text_proj_in_factor %||% 49L, video_connector_num_attention_heads = config$video_connector_num_attention_heads %||% 30L, video_connector_attention_head_dim = config$video_connector_attention_head_dim %||% 128L, video_connector_num_layers = config$video_connector_num_layers %||% 2L, video_connector_num_learnable_registers = as.integer(config$video_connector_num_learnable_registers), audio_connector_num_attention_heads = config$audio_connector_num_attention_heads %||% 30L, audio_connector_attention_head_dim = config$audio_connector_attention_head_dim %||% 128L, audio_connector_num_layers = config$audio_connector_num_layers %||% 2L, audio_connector_num_learnable_registers = as.integer(config$audio_connector_num_learnable_registers), connector_rope_base_seq_len = config$connector_rope_base_seq_len %||% 4096L, rope_theta = config$rope_theta %||% 10000.0, rope_double_precision = config$rope_double_precision %||% TRUE, causal_temporal_positioning = config$causal_temporal_positioning %||% FALSE, rope_type = config$rope_type %||% "split")
    } else {
        # Load weights early to auto-detect learnable registers
        if (verbose) message("Loading weights from: ", weights_path)
        weights <- safetensors::safe_load_file(weights_path, framework = "torch")

        # Auto-detect learnable registers from weight keys
        video_reg <- NULL
        audio_reg <- NULL
        for (k in names(weights)) {
            if (grepl("video.*learnable_registers$", k)) {
                video_reg <- as.integer(weights[[k]]$shape[1])
            }
            if (grepl("audio.*learnable_registers$", k)) {
                audio_reg <- as.integer(weights[[k]]$shape[1])
            }
        }

        connectors <- ltx2_text_connectors(video_connector_num_learnable_registers = video_reg, audio_connector_num_learnable_registers = audio_reg)
    }

    # Load weights (may already be loaded for auto-detection)
    if (!exists("weights", inherits = FALSE)) {
        if (verbose) message("Loading weights from: ", weights_path)
        weights <- safetensors::safe_load_file(weights_path, framework = "torch")
    }

    # If separate text_proj_path provided, merge those weights in
    if (!is.null(text_proj_path)) {
        if (!file.exists(text_proj_path)) {
            stop("Text projection file not found: ", text_proj_path)
        }
        if (verbose) message("Loading text projection from: ", text_proj_path)
        proj_weights <- safetensors::safe_load_file(text_proj_path, framework = "torch")
        for (k in names(proj_weights)) {
            weights[[k]] <- proj_weights[[k]]
        }
    }
    ...
    # Move to device
    torch_dtype <- switch(dtype, "float32" = torch::torch_float32(), "float16" = torch::torch_float16(), "bfloat16" = torch::torch_bfloat16(), torch::torch_float32())
...
load_ltx2_connector_weights <- function(connectors, weights, verbose = TRUE) {
    ...
    remap_connector_key <- function(key) {
        # Handle Wan2GP format: strip diffusion_model. prefix
        key <- sub("^diffusion_model\\.", "", key)

        # Wan2GP uses {video,audio}_embeddings_connector, R uses {video,audio}_connector
        key <- sub("^video_embeddings_connector\\.", "video_connector.", key)
        key <- sub("^audio_embeddings_connector\\.", "audio_connector.", key)

        # Wan2GP uses transformer_1d_blocks, R uses transformer_blocks
        key <- gsub("\\.transformer_1d_blocks\\.", ".transformer_blocks.", key)

        # Wan2GP uses q_norm/k_norm, R uses norm_q/norm_k
        key <- gsub("\\.q_norm\\.", ".norm_q.", key)
        key <- gsub("\\.k_norm\\.", ".norm_k.", key)

        # Wan2GP text_proj: text_embedding_projection.aggregate_embed.weight -> text_proj_in.weight
        key <- sub("^text_embedding_projection\\.aggregate_embed\\.", "text_proj_in.", key)
