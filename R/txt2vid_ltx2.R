...
#' @param text_backend Character. Text encoding backend: "api", "precomputed", or "random".
...
txt2vid_ltx2 <- function(
  ...
  torch::with_no_grad({
    ...
    text_result <- encode_text_ltx2(
      ...
      caption_channels = 3840L  # LTX-2 default
    )
    ...
    prompt_embeds <- prompt_embeds$to(device = dit_device, dtype = latent_dtype)
    negative_prompt_embeds <- negative_prompt_embeds$to(device = dit_device, dtype = latent_dtype)
    ...
    # NOTE: For now, we assume DiT is passed in or we're in test mode
    # Full weight loading will be added later
    if (is.null(dit)) {
      if (verbose) message("NOTE: DiT model not provided - using placeholder")
      # In real usage, this would load from safetensors
      # For now, create a small test model
    ...
    audio_latents <- torch::torch_zeros(
      c(batch_size, 50L, 64L),  # Placeholder audio
      device = dit_device,
      dtype = latent_dtype
    )
    ...
    for (i in seq_len(num_inference_steps)) {
      ...
      sigma <- sigmas[t_idx]
      sigma_next <- if (i < num_inference_steps) sigmas[t_idx + 1L] else 0
      ...
      if (memory_profile$cfg_mode == "sequential") {
        ...
        noise_pred <- sequential_cfg_forward(
          ...
          audio_encoder_hidden_states = prompt_embeds,  # Shared for now
