...
#' }
ltx2_memory_profile <- function (vram_gb = NULL, model = "ltx2-19b-fp4") {
...
is_blackwell_gpu <- function () {
    ...
    # cuda_get_device_properties may not exist in all torch versions
    if (torch::cuda_is_available()) {
        get_props <- tryCatch(
            get("cuda_get_device_properties", envir = asNamespace("torch")),
            error = function (e) NULL
        )
        props <- if (!is.null(get_props)) tryCatch(get_props(0L), error = function(e) NULL)
        if (!is.null(props)) {
...
#' \itemize{
#'   \item \code{<name>::packed} - uint8 tensor with packed INT4 values
#'   \item \code{<name>::scales} - float32 tensor with per-block scales
#'   \item \code{<name>::shape} - int64 tensor with original shape
#' }
#'
