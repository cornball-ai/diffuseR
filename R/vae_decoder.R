...
VAEUpBlock <- torch::nn_module(
 ...
 initialize = function(in_channels, out_channels, num_resnets = 3, add_upsample = TRUE) {
   self$resnets <- torch::nn_module_list()

   for (i in seq_len(num_resnets)) {
     res_in <- if (i == 1) in_channels else out_channels
...
#' @export
load_decoder_weights <- function(native_decoder, torchscript_path, verbose = TRUE) {
...
vae_decoder_native <- torch::nn_module(
 ...
 initialize = function(latent_channels = 4, out_channels = 3) {
