...
bpe_tokenizer <- function(tokenizer_path) {
    ...
    if (!is.null(added_tokens)) {
        for (tok in added_tokens) {
            special_tokens[[tok$content]] <- list(id = tok$id,
...
encode_bpe <- function(tokenizer, text, add_special_tokens = TRUE,
    ...
    if (return_tensors == "pt") {
        ...
        attention_mask <- torch::torch_tensor(attention_mask,
...
encode_single <- function(tokenizer, text, add_special_tokens = TRUE) {
    ...
    ids <- greedy_tokenize(text, tokenizer$vocab, tokenizer$byte_fallback,
                           tokenizer$unk_token)
...
#' @keywords internal
greedy_tokenize <- function(text, vocab, byte_fallback = FALSE,
                            unk_token = NULL) {
    ids <- integer(0)
    i <- 1
    n <- nchar(text)
            ...
            if (byte_fallback) {
                ...
                if (char %in% vocab_names) {
                    ...
                    for (b in bytes) {
                        ...
                        } else if (!is.null(unk_token) &&
                            unk_token %in% vocab_names) {
