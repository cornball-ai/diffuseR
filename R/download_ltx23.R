...
download_ltx2 <- function(
  ...
  result <- list(checkpoint = NULL, fp8_dir = output_dir, text_encoder_dir = NULL)
  ...
  if (!have_fp8 || !quantize) {
    ...
    if (is.null(cached) && !have_fp8) {
      ...
      if (!ok) stop("Download cancelled.", call. = FALSE)
      if (verbose) message("Downloading ", .ltx23_checkpoint_file, " (46 GB)...")
      cached <- hfhub::hub_download(.ltx23_checkpoint_repo, .ltx23_checkpoint_file)
    }
    result$checkpoint <- cached

    if (quantize && !have_fp8) {
      if (verbose) message("Quantizing transformer linears to fp8 (one-time)...")
  ...
  if (text_encoder) {
    ...
    if (verbose && !have_te) message("Downloading Gemma3 text encoder...")
